...
Express serves the website API and Vite during local development. The server uses Firebase Admin SDK for privileged Firestore access. The browser calls Express; service-account credentials and the admin token stay on the server.
...
3. Set `GOOGLE_APPLICATION_CREDENTIALS=./serviceAccountKey.json` in `.env`. The provided local `.env` already points there. Set `FIREBASE_PROJECT_ID` and a private `ADMIN_API_TOKEN` as well.
...
6. Run the app with `npm run dev`, open `http://localhost:3000`, and sign in to `/admin` with `ADMIN_API_TOKEN`.
...
2. In Vercel Project Settings → Environment Variables, add these to **Production**:
   ...
   - `ADMIN_API_TOKEN` — a unique, randomly generated secret; never use the local token or commit it
...
Firestore rules default to deny. The API uses the Admin SDK, which bypasses client rules; protect the server credential and `ADMIN_API_TOKEN`. The initial admin release still uses a single shared token. Per-user accounts, payment sessions/webhooks, and a visual homepage editor are not implemented yet.
...
- Admin APIs are under `/api/v1/admin` and require `Authorization: Bearer <ADMIN_API_TOKEN>`.
